...
declare
  cursor Drop_users is
    select *
      from SNDB_USER_LOGIN;
              drop_sql varchar2(100);
        USER_name varchar2(100);
      
begin
 for C in Drop_users LOOP
  USER_name:= C.USERNAME; 
 
   drop_sql:='DROP USER '||USER_name||' CASCADE';
    EXECUTE IMMEDIATE drop_sql;
   dbms_output.put_line('Dropped USER : ' || C.USERNAME);
...
BEGIN
    ...
    ')]';
	 ...
	 EXECUTE IMMEDIATE q'[INSERT INTO CONFIG_TABLE VALUES ('SNDB_ROLES_PERMISSIONS','CREATE TABLE SNDB_ROLES_PERMISSIONS
            (	
            ROLES_ID NUMBER(10) GENERATED BY DEFAULT AS IDENTITY, 
            ROLES_DESCRIPTION VARCHAR2(100) NOT NULL,
            CONSTRAINT SNDB_ROLES_PK PRIMARY KEY(ROLES_ID))
    ')]';
    
     EXECUTE IMMEDIATE q'[INSERT INTO CONFIG_TABLE VALUES ('SNDB_GROUPS_OF_ROLES','  CREATE TABLE SNDB_GROUPS_OF_ROLES
            (
             GROUPS_ROLES_ID NUMBER(10) GENERATED BY DEFAULT AS IDENTITY, 
             GROUPS_ID NUMBER(10), 
             ROLES_ID NUMBER(10), 
             CONSTRAINT SNDB_GROUPS_ROLES_PK PRIMARY KEY(GROUPS_ROLES_ID),
             CONSTRAINT SNDB_GROUPS_ROLES_FK_GROUPS FOREIGN KEY (GROUPS_ID) REFERENCES SNDB_USER_ROLES_DATA(role_id),
             CONSTRAINT SNDB_GROUPS_ROLES_FK_ROLES FOREIGN KEY (ROLES_ID) REFERENCES SNDB_ROLES_PERMISSIONS(ROLES_ID) )
    ')]';
...
PROCEDURE SNDB_ASSIGN_PERMISSION_TO_ROLES(U_USER_NAME VARCHAR, GROUP_NAME VARCHAR);
...
EXECUTE IMMEDIATE 'CREATE USER ' || i_username || ' IDENTIFIED BY ' || i_password;
   DBMS_OUTPUT.PUT_LINE('CREATE USER ' || i_username || ' IDENTIFIED BY ' || i_password);
   if upper(i_user_type) = 'USER' then
        INSERTIONS.SNDB_ASSIGN_PERMISSION_TO_ROLES(i_username, 'USER');
  elsif upper(i_user_type) = 'REPORT ANALYST' then
        INSERTIONS.SNDB_ASSIGN_PERMISSION_TO_ROLES(i_username, 'REPORT ANALYST');
        elsif upper(i_user_type) = 'SOCIAL MEDIA MANAGER' then
        INSERTIONS.SNDB_ASSIGN_PERMISSION_TO_ROLES(i_username, 'SOCIAL MEDIA MANAGER');
end if;
if user_count=0 then
...
GRANT_sql varchar2(100);
 ...
 CURSOR some_cur
  IS
    ...
    where g.role_name = UPPER(GROUP_NAME);
...
create or replace PROCEDURE ADD_ROLES_PERMISSIONS(L_ROLES_DESCRIPTION IN VARCHAR2)
    AS
    ...
    BEGIN  
        ...
        MERGE_STMT_SQL:= 'MERGE INTO SNDB_ROLES_PERMISSIONS R USING ' || USING_STMT || 'TEMP ON (
      ...
      commit;  
    END ADD_ROLES_PERMISSIONS;
    
/

 create or replace PROCEDURE ADD_GROUP_ROLES(L_GROUPS_ID IN NUMBER, L_ROLES_ID IN NUMBER)
    AS
    MERGE_STMT_SQL VARCHAR2(500);
    USING_STMT VARCHAR2(500);
    i_group_id number;
    ...
    raise role_id_doesnt_exsists_in_roles_permissions_data;
        ...
        MERGE_STMT_SQL:= 'MERGE INTO SNDB_GROUPS_OF_ROLES GR USING ' || USING_STMT || 'TEMP ON (GR.GROUPS_ID =  TEMP.GROUPS_ID 
...
commit;
end sndb_update_logged_in_data;
...
begin
if i_payment_request_id is null then;
...
commit;
end SNDB_UPDATE_payment_request_data;
...
commit;
end sndb_update_addfriend_data;
...
create or replace view SNDB_view_friends as
...
create or replace view SNDB_StudentViewSDC as
...
--------------Student specific--------------------------
execute ADD_ROLES_PERMISSIONS('CREATE SESSION');
execute ADD_ROLES_PERMISSIONS('SELECT ON SNDB_View_Student_details');
execute ADD_ROLES_PERMISSIONS('UPDATE ON sndb_user_login');
execute ADD_ROLES_PERMISSIONS('UPDATE ON sndb_user_account');
execute ADD_ROLES_PERMISSIONS('EXECUTE ANY PROCEDURE');
execute ADD_ROLES_PERMISSIONS('UPDATE ON SNDB_View_Student_details');
execute ADD_ROLES_PERMISSIONS('UPDATE ON SNDB_View_Student_profile_photo');
execute ADD_ROLES_PERMISSIONS('SELECT ON SNDB_View_Student_profile_photo');
execute ADD_ROLES_PERMISSIONS('UPDATE ON SNDB_View_Student_Post_Details');
execute ADD_ROLES_PERMISSIONS('SELECT ON SNDB_View_Student_Post_Details');
execute ADD_ROLES_PERMISSIONS('UPDATE ON SNDB_view_friends');
execute ADD_ROLES_PERMISSIONS('SELECT ON SNDB_view_friends');
execute ADD_ROLES_PERMISSIONS('SELECT ON SNDB_View_Student_Payment_Details');
execute ADD_ROLES_PERMISSIONS('UPDATE ON SNDB_View_Student_Payment_Details');
execute ADD_ROLES_PERMISSIONS('SELECT ON SNDB_view_Student_group_message');
execute ADD_ROLES_PERMISSIONS('SELECT ON SNDB_StudentViewSDC');
execute ADD_ROLES_PERMISSIONS('UPDATE ON SNDB_StudentViewSDC');



--------------------------Report Analyst--------------------------
execute ADD_ROLES_PERMISSIONS('SELECT ON sndb_user_account');
execute ADD_ROLES_PERMISSIONS('SELECT ON sndb_user_login');
execute ADD_ROLES_PERMISSIONS('SELECT ON sndb_post_data');
execute ADD_ROLES_PERMISSIONS('SELECT ON sndb_conversation_data');
execute ADD_ROLES_PERMISSIONS('SELECT ON sndb_User_Roles_data');
execute ADD_ROLES_PERMISSIONS('SELECT ON sndb_group');
execute ADD_ROLES_PERMISSIONS('SELECT ON sndb_user_account_group');
execute ADD_ROLES_PERMISSIONS('SELECT ON sndb_payment_request_data');
execute ADD_ROLES_PERMISSIONS('SELECT ON sndb_message_data');


---------------------Social Media Manager-------------------------
execute ADD_ROLES_PERMISSIONS('SELECT ON SNDB_View_Social_Media_Manager_Post_Details');
execute ADD_ROLES_PERMISSIONS('SELECT ON SNDB_View_Social_Media_Manager_Photo_Data');



execute ADD_GROUP_ROLES(1,1);
execute ADD_GROUP_ROLES(1,2);
execute ADD_GROUP_ROLES(1,3);
execute ADD_GROUP_ROLES(1,4);
execute ADD_GROUP_ROLES(1,5);
execute ADD_GROUP_ROLES(1,6);
execute ADD_GROUP_ROLES(1,7);
execute ADD_GROUP_ROLES(1,8);
execute ADD_GROUP_ROLES(1,9);
execute ADD_GROUP_ROLES(1,10);
execute ADD_GROUP_ROLES(1,11);
execute ADD_GROUP_ROLES(1,13);
execute ADD_GROUP_ROLES(1,14);
execute ADD_GROUP_ROLES(1,15);
execute ADD_GROUP_ROLES(1,12);
execute ADD_GROUP_ROLES(1,16);
execute ADD_GROUP_ROLES(1,17);

execute ADD_GROUP_ROLES(3,1);
execute ADD_GROUP_ROLES(3,18);
execute ADD_GROUP_ROLES(3,19);
execute ADD_GROUP_ROLES(3,20);
execute ADD_GROUP_ROLES(3,21);
execute ADD_GROUP_ROLES(3,22);
execute ADD_GROUP_ROLES(3,23);
execute ADD_GROUP_ROLES(3,24);
execute ADD_GROUP_ROLES(3,25);
execute ADD_GROUP_ROLES(3,26);

execute ADD_GROUP_ROLES(4,1);
execute ADD_GROUP_ROLES(4,27);
execute ADD_GROUP_ROLES(4,28);

-------------------------------------


EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Margaux','ZEVLO_Spring_732', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Forrest','FRKOI_Spring_481', 'USER');
...
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Lorrayne','XNIWC_Spring_427', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Eberhard','LTWOQ_Spring_820', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Maximilianus','NDUXV_Spring_138', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Marina','ENZWD_Spring_915', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Moreen','MHQVM_Spring_252', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Reinold','LSCEG_Spring_219', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Mattheus','AHYEI_Spring_804', 'REPORT ANALYST');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Gabriela','YTJDM_Spring_241', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Werner','ORKUE_Spring_713', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Randee','TBISW_Spring_468', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Elise','IVYNE_Spring_632', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Skipper','SDVIZ_Spring_394', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Willi','FBFLF_Spring_249', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Elena','QHVWO_Spring_878', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Lianne','JQQIR_Spring_764', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Stephanie','ACABW_Spring_151', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Kendell','DBXXD_Spring_533', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Lianna','FFBRT_Spring_166', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Penni','GRAJU_Spring_219', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Cory','FMRLS_Spring_437', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Pincus','IIFDI_Spring_079', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Justina','EETEY_Spring_493', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Gretel','AKBZN_Spring_244', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Lukas','CNIYH_Spring_702', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Tate','UMUJU_Spring_993', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Mohandis','EFWMA_Spring_359', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Betty','VPYWM_Spring_618', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Herculie','CUEFN_Spring_317', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Lew','NUUNM_Spring_281', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Sal','FQXBR_Spring_119', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Jeana','MCBTZ_Spring_664', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Mala','MJGSF_Spring_920', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Elbertine','KXLVE_Spring_387', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Ursala','YMXLP_Spring_724', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Jayme','SUNMN_Spring_987', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Devon','CLLQY_Spring_622', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Alfie','OPQRB_Spring_808', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Cassandra','PEBZK_Spring_796', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Brynna','YXBQU_Spring_463', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Kaia','MMFBM_Spring_087', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Teador','CCMNZ_Spring_072', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Randolph','NXYTV_Spring_483', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Marjorie','FGNJT_Spring_290', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Ericka','JQMAK_Spring_308', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Madelyn','WWYFE_Spring_137', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Stu','POKSR_Spring_485', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Kevon','QKITR_Spring_168', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Gisela','HEAGW_Spring_136', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Mason','SJFNG_Spring_954', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Mischa','WZFUC_Spring_377', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Jereme','HVVBS_Spring_999', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Olympie','JHBQW_Spring_587', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Valaree','RZRAA_Spring_567', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Vern','UCLST_Spring_529', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Katie','ZZMIZ_Spring_408', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Ramonda','JFLEK_Spring_167', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Teddie','DGMHW_Spring_257', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Jonathan','CBQMJ_Spring_086', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Lani','MWSBI_Spring_419', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Domenic','GGTLK_Spring_881', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Inge','HMKXW_Spring_424', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Crissie','AFEPN_Spring_058', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Ignaz','ELIYB_Spring_063', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Emilia','KHGAM_Spring_029', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Lory','OCCKS_Spring_696', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Raddy','AYGRV_Spring_501', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Octavia','MWIUW_Spring_231', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Mara','ZKDEN_Spring_291', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Giovanni','JXHYO_Spring_450', 'USER');
EXECUTE INSERTIONS.SNDB_ADD_USER_LOGIN('Allegra','JUTKE_Spring_564', 'USER');
