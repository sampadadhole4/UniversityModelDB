--create user new_tester_user1 identified by Spring_team_123;
 SET SERVEROUTPUT ON;
...
BEGIN
    ...
    (
       ...
       CONSTRAINT SNDB_CONFIG_TABLE_PK PRIMARY KEY(TABLE_NAME)
    )';
    ...
    ')]';
       ...
       CONSTRAINT SNDB_user_login_id_pk PRIMARY KEY (user_login_id),
        CONSTRAINT SNDB_FK_user_type FOREIGN KEY (user_type)
...
CONSTRAINT SNDB_gender_id_pk PRIMARY KEY (gender_id ),
 CONSTRAINT SNDB_gender_name_allowed CHECK (name IN (''MALE'',''FEMALE'',''OTHERS''))
    ...
    ')]';
     ...
     user_account_id number not null,
      ...
      CONSTRAINT SNDB_user_id_pk PRIMARY KEY (user_account_id),
  CONSTRAINT SNDB_FK_user_id FOREIGN KEY (gender_id)
   REFERENCES SNDB_gender_data(gender_id),
   CONSTRAINT SNDB_FK_user_login_id
...
(
 ...
 CONSTRAINT SNDB_friendship_id_pk PRIMARY KEY (friendship_id ),
       CONSTRAINT SNDB_FK_requester_id FOREIGN KEY (requester_id)
    REFERENCES SNDB_user_account(user_id),
        CONSTRAINT SNDB_FK_addressee_id FOREIGN KEY (addressee_id)
    REFERENCES SNDB_user_account(user_id)
  )
')]';
...
(
   ...
   EXECUTE IMMEDIATE q'[INSERT INTO CONFIG_TABLE VALUES ('SNDB_group_recipient','CREATE TABLE SNDB_group_recipient 
       ...
       CONSTRAINT SNDB_group_recipnt_id_pk PRIMARY KEY (group_recipient_id ),
...
dbms_output.put_line('entered creator id : = ' || i_creator_id || ' exists in the table');
when group_message_id_exists then
dbms_output.put_line('entered group message id : = ' || i_group_message_id || ' exists in the table');
...
--    SELECT 
--        g.role_name, r.roles_description
--    FROM  SNDB_USER_ROLES_DATA g
--        join SNDB_GROUPS_OF_ROLES gr on g.ROLE_id = gr.groups_id
--        join SNDB_ROLES_PERMISSIONS r on r.roles_id = gr.roles_id
--    where g.role_name = 'user';
--    
